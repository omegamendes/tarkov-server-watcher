	...
	go func() {
		for {
			...
			case <-mShowIP.ClickedCh:
				if latestIP != "" {
					systray.SetTitle(fmt.Sprintf("Latest IP: %s | Last Updated: %s", latestIP, lastUpdated))
					fmt.Println("Latest IP:", latestIP, "| Last Updated:", lastUpdated)
				} else {
...
func checkIPAndUpdateTray(latestIP *string, lastUpdated *string) {
	...
	*lastUpdated = time.Now().Format("15:04")
	...
	systray.SetTooltip(fmt.Sprintf("Latest IP: %s | Country: %s | Last Updated: %s", *latestIP, countryName, *lastUpdated))
}
